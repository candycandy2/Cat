$("#viewPayShopReturnReason").pagecontainer({
    create: function(event, ui) {
        ...
        function initReasonSelect() {
            ...
            tplJS.DropdownList("viewPayShopReturnReason", "qpayReturnReason", "prepend", "typeB", reasonData);
            //设置默认值0
        ...
        function getTradeDetail() {
            let tradeInfo = JSON.parse(window.sessionStorage.getItem('viewPayShopReturnReason_parmData'));
        ...
        /********************************** page event ***********************************/
        $("#viewPayShopReturnReason").on("pagebeforeshow", function(event, ui) {

        });

        $("#viewPayShopReturnReason").one("pageshow", function(event, ui) {
            var mainHeight = window.sessionStorage.getItem('pageMainHeight');
            $('#viewPayShopReturnReason .page-main').css('height', mainHeight);
            $('.return-reason-row3').show();
            initReasonSelect();
        });

        $("#viewPayShopReturnReason").on("pageshow", function(event, ui) {
            getTradeDetail();
        });

        $("#viewPayShopReturnReason").on("pagehide", function(event, ui) {
        ...
        $('.returnToPassword').on('click', function() {
            ...
            if(has) {
                ...
                checkWidgetPage('viewPayShopReturnPassword', pageVisitedList, tradeInfo);
            }
        });
