...
$(function() {
    ...
    function initialAPP() {
        ...
        if (isMobile) {
            $("#viewRecord .record-content .record-body > .record").html("");
        } else {
            $("#viewRecord .record-content record-table.desktop tbody > tr").html("");
        }
    }
    initialAPP();
    ...
    if (window.mobilecheck()) {
        ...
        $(".page").each(function(index, dom) {
            if ($(dom).prop("id") != "viewRecord") {
                $(dom).css("min-height", document.documentElement.clientHeight + "px");
            }
        });
