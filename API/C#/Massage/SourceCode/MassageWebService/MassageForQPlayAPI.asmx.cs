...
namespace MassageWebService
{
    /// <summary>
    /// MassageForQPlayAPI 的摘要说明
    /// 20170724
    /// Hakkinen Coding
    /// </summary>
    ...
    public class MassageForQPlayAPI : System.Web.Services.WebService
    {
        // 我的預約
        [WebMethod]
        public string QueryMyReserve(string strXml)
        {
            ...
            try
            {
                ...
                #region 輸入參數說明
                /*
                    <LayoutHeader><ReserveUser>1501005</ReserveUser><NowDate>20170727</NowDate></LayoutHeader>
				*/
                #endregion
                ...
                if (!string.IsNullOrEmpty(ReserveUser) && !string.IsNullOrEmpty(NowDate))
                {
                    ...
                    // 我的預約 SQL Script
                    // Site 註釋 : M-桃園 N-新竹 T-台北
                    string SQL = " Select Case When ClassIndex = 'T' then 'QTT' When ClassIndex = 'M' then 'QTY' When ClassIndex = 'N' then 'QTH' Else '' End As Site, " 
                               + " Convert( Nvarchar, STime, 112 ) as 'ReserveDate', STime as 'ReserveBeginTime', ETime as 'ReserveEndTime', OWVID as 'ReserveID' "
                               + " From Massage_Booking_Date "
                               + " Where 1=1 "
                    ...
                    else
                    {
                        // 查無預約資料
                        strJson = new JObject(
                                    new JProperty("ResultCode", "039901"),
                                    new JProperty("Content", "")
                                    ).ToString();
                    }
                }
        ...
        public string QueryReserveDetail(string strXml)
        {
            ...
            try
            {
                ...
                #region 輸入參數說明
                /*
                <LayoutHeader><Site>QTY</Site><ReserveDate>20170522</ReserveDate></LayoutHeader>
        		*/
                #endregion
                ...
                if (!string.IsNullOrEmpty(Site) && !string.IsNullOrEmpty(ReserveDate))
                {
                    ...
                    string SQL = " Select isnull( RV.Nam,'' )  as 'Name', "
                               ...
                               //  20170725 Hakkinen 修正今天時間已過卻仍可預約
        ...
        // 預約
        [WebMethod]
        public string ReserveMassage(string strXml)
        {
            string strJson = "";
            try
            {
        		...
        		*/
                ...
                if (!string.IsNullOrEmpty(Site) && !string.IsNullOrEmpty(ReserveDate) && !string.IsNullOrEmpty(ReserveUser) && !string.IsNullOrEmpty(BTime))
                {
                    ...
                    int ReserveNum = Substrings.Length;
                    string SQL = "";
                    for (int i = 0; i <= Substrings.Length - 1; i++)
                    {
                        SQL += " Select RV.Nam as 'Name', RV.Telno as 'Ext_No' ,RV.Email as 'EMail' ,Convert( Nvarchar(5), RV.STime, 114 ) as 'BTime' ,Convert( int, RV.Status ) as 'Status' ,RV.OWVID as 'ReserveID'  "
                                   + " From Massage_Booking_Date_View RV "
                                   + " Where 1=1 "
                                   + " And status = 1 "
                                   + " And Site = @Site "
                                   + " And Convert( Nvarchar, RV.STime, 112 ) = @ReserveDate "
                    ...
                    if (DataTableReserveDetailInfo != null && DataTableReserveDetailInfo.Rows.Count > 0)
                    {
                        ...
                        // 回傳預約失敗，有人預約
                        strJson = new JObject(
                                    new JProperty("ResultCode", "039904")
                                                  ).ToString();
                    }
                    else
                    {
                        ...
                        {
                            ReserveNum = ReserveNum + (int)DataTableReserveCountInfo.Rows[0]["BCount"];
                        }

                        // 判斷是否有超過預約限制
                        // 20170728 Hakkinen 台北 桃園 2小時(4個時段) 新竹 4小時(8個時段)
                        if ( ( Site == "QTH" && ReserveNum > 8 ) || ( Site == "QTY" && ReserveNum > 4 ) || ( Site == "QTT" && ReserveNum > 4 ) )
                        {
                            // 關閉連結 SQLConn
                            SQLConn.Close();
                            // 預約失敗，已超出可預約的時數限制
                            strJson = new JObject(
                                        new JProperty("ResultCode", "039903")
                                                      ).ToString();
                        }
                        else
                        {
                            // 設定執行參數
                            cmd = new SqlCommand("usp_Massage_Booking", SQLConn);
                            ...
                            // 預約成功
                            strJson = new JObject(
                                        new JProperty("ResultCode", "039902")
                                                 ).ToString();
                        }
                    }
                }
            ...
            try
            {
                ...
                */
                ...
                if (!string.IsNullOrEmpty(ReserveDate) && !string.IsNullOrEmpty(ReserveUser) && !string.IsNullOrEmpty(ReserveID))
                {
                    ...
                    if (intTotal != 0)
                    {
                        string SQLUpdate = " Update Massage_Booking_Date set Status = 0 "
                                         ...
                                         + " From Massage_Booking_List AS RBL Inner join Memberall_Simple AS MS on RBL.EmpNo = MS.Emp_No "
                       ...
                       // 取消預約成功
                       strJson = new JObject(
                                     new JProperty("ResultCode", "039905")
                                     ).ToString();
                       // 關閉 SQLConn
                       SQLConn.Close();                    
                    }
                    else
                    {
                        // 取消預約失敗,當日內預約無法取消
                        strJson = new JObject(
                                      new JProperty("ResultCode", "039906")
                                      ).ToString();
                    }
                 }
                else
