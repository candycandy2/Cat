var Messages = {
    ERROR : 'Error',
    MESSAGE : 'Message',
    REMOVE_CONFIRM : 'Remove Confirmation',
    MSG_OPERATION_SUCCESS : 'Operation Success!',
    MSG_OPERATION_FAILED : 'Operation Failed!',
    MSG_DEFAULT_LANGUAGE_CAN_NOT_REMOVE : 'The default language can not be deleted',
    MSG_CONFIRM_UNPUBLISH_VERSION : 'Confirm to unpublish %s',
    MSG_CONFIRM_UNPUBLISH : 'Confirm Unpublish',
    MSG_CONFIRM_PUBLISH_STATUS : 'Confirm %s publish to Android- %l | IOS- %k？',
    MSG_CONFIRM_PUBLISH : 'Confirm Publish',
    MSG_SAVE_FAILED : 'Save Failed,please checked',
    MSG_SYSTEM_WILL_DELETE_ALL_ALNGUAGE : 'System will delete all information about the language,comfirm to delete?',
    ERR_DUPLICATE_API_ACTION : 'The same version of the API action already exists',
    ERR_SCREENSHOT_FILE_LIMIT : 'The screenshot can only upload %s sheets. You have selected %l sheets',
    ERR_SCREENSHOT_SCALE_LIMIT : 'File size, accepted only %s * %l',
    ERR_QPLAY_MUST_PUBLISH : 'QPlay must have a published version,please set %s published',
    ERR_ILLEGAL_INSERT_HTML : 'Illigal to insert html code',
    CUMULATIVE_REGISTERED : 'Cumulative registration',
    TOKEN_VALID : 'Token is valid',
    DEVICE_PERCENT : 'Devices ratio',
    USER_PERCENT : 'Users ratio',
    BY_DEVICE : 'each devices',
    BY_COMPANY_SITE : 'each company + site',
    CALL_USERS_RATE : 'Calls By Users',
    CALL_TIMES_RATE : 'Calls By Times',
    DAY : '1 Day',
    N_MONTH : '%d Month',
    BY_DEPARTMENT : 'each department',
    AVG_OPERATION_TIME : 'AVG Operation Time',
    NO_DATA_TO_DISPLAY : 'No data to display',
    AVG :　'AVG',
    OPERATION_TIME : 'Operation Time',
    MAX_STAY_TIME : 'Max Stay Time',
    DAILY_REGISTERED_DEVICES : 'Daily Registered Devices',
    DAILY_REGISTERED_USERS : 'Daily Registered Users',
    REGISTERED_DEVICES :'Registered Devices',
    REGISTERED_USERS :'Registered Users',
    CUMULATIVE_REGISTERED_DEVICE : 'Cumulative Registered Devices',
    CUMULATIVE_REGISTERED_USERS : 'Cumulative Registered Users',
    CUMULATIVE_REGISTERED_DEVICE_RATE : 'Cumulative Registered Devices Rate',
    CUMULATIVE_REGISTERED_USERS_RATE : 'Cumulative Registered Users Rate'

}